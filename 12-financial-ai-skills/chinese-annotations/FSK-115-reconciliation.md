...
- `1. Reconciliation Types and Hierarchy`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `2. Three-Way Reconciliation`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `3. Break Identification`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `4. Break Resolution Workflows`（三级标题）：这是实际操作流程，建议逐步对照输入、处理、输出和人工复核点。
...
`reconciliation` 的注释只解释该 skill 的用途、结构和迁移方向，不与其他同名或相近 skill 合并。
