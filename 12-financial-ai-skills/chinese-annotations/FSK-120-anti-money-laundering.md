# FSK-120 `anti-money-laundering` 中文详细注释

## 1. 基本信息

- **Skill 编号**：FSK-120
- **原始名称**：`anti-money-laundering`
- **所属领域**：证券合规、反洗钱与监管要求
- **来源项目**：`community-joellewis-finance-skills`
- **原始路径**：`12-financial-ai-skills/upstream-snapshots/community-joellewis-finance-skills/plugins/compliance/skills/anti-money-laundering/SKILL.md`

## 2. 这个 skill 是干什么的

原始描述可以理解为：Guide BSA/AML compliance program design and operation for broker-dealers, banks, and investment advisers. Use when the user asks about suspicious activity reports, currency transaction reports, OFAC screening, structuring detection, or FinCEN requirements. Also trigger when users mention 'large cash deposit', 'sanctions check', 'money laundering red flags', 'customer risk rating', 'unusual transaction patterns', 'wire to a foreign country', 'SDN list', 'tipping off a client about a SAR', 'AML audit', 'correspondent account due diligence', or ask whether a transaction needs to be reported.

合规 skill 覆盖 KYC、AML、Reg BI、受托责任、广告合规、信息披露、记录保存、隐私和监管检查等美国证券监管主题。

放到中文语境里，`anti-money-laundering` 不是一个独立应用，而是一段可以被 AI 助手调用的专业工作流。它把原来依赖人工经验的金融任务拆成更明确的触发条件、输入材料、处理步骤、输出格式和质量检查点。你阅读它时，不要只看英文标题，而要看它如何规定“先收集什么、再判断什么、最后交付什么”。
...
- 当你以后要把银行经验改造成中文 skill、提示词模板或自动化实验时，可以把 `anti-money-laundering` 作为结构参考。
...
- `Anti-Money Laundering Compliance`（一级标题）：这是原始 skill 的结构节点，可作为阅读导航。
...
- `Bank Secrecy Act (BSA) Framework`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `FINRA Rule 3310 — AML Compliance Program`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `Currency Transaction Reports (CTRs)`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `Suspicious Activity Reports (SARs)`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。

这些标题相当于原始 skill 的骨架。建议阅读顺序是：先看触发条件，再看步骤，再看示例，最后看风险/限制/参考文件。这样能避免只记住术语，却没有理解真实金融工作流。

## 7. 金融 + AI 的核心特点

`anti-money-laundering` 的核心特点，是把金融专业判断前面的准备工作结构化。AI 最适合做资料整理、格式化、初步检查、摘要、草稿、差异定位和问题清单；但金融结论本身仍然依赖数据质量、制度规则、业务经验和人工复核。
...
- AML 指反洗钱，关注客户风险、交易监测、可疑行为、制裁筛查和报告义务。
...
第一遍只读中文注释，理解 `anti-money-laundering` 的业务作用。第二遍打开原始 `SKILL.md`，对照每个标题看它怎样组织步骤。第三遍尝试把它改写成一个中文银行场景，例如“客户经理会前准备”“对账差错分析”“KYC 材料缺口识别”“财报摘要”或“市场早报”。
...
`anti-money-laundering` 的注释只解释该 skill 的工作流，不重复 01 模块里反洗钱/合规的制度总论；如果后续本地化，应另写中国银行业版本。
