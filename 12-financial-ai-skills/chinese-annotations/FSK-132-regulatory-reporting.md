# FSK-132 `regulatory-reporting` 中文详细注释

## 1. 基本信息

- **Skill 编号**：FSK-132
- **原始名称**：`regulatory-reporting`
- **所属领域**：证券合规、反洗钱与监管要求
- **来源项目**：`community-joellewis-finance-skills`
- **原始路径**：`12-financial-ai-skills/upstream-snapshots/community-joellewis-finance-skills/plugins/compliance/skills/regulatory-reporting/SKILL.md`

## 2. 这个 skill 是干什么的

原始描述可以理解为：Guide regulatory filing obligations and deadlines for investment advisers, broker-dealers, and large traders. Use when the user asks about Form PF filing thresholds, 13F institutional holdings reports, 13H large trader filings, Form ADV amendment timing, FOCUS report preparation, blue sheet requests, CAT reporting infrastructure, or FINRA short interest and TRACE reporting. Also trigger when users mention 'filing deadline calendar', 'do we need to file Form PF', 'crossed the $100M 13F threshold', 'annual updating amendment', 'CAT clock synchronization', 'how to respond to a blue sheet request', 'FOCUS report errors', or ask which regulatory filings a firm must make and when.

合规 skill 覆盖 KYC、AML、Reg BI、受托责任、广告合规、信息披露、记录保存、隐私和监管检查等美国证券监管主题。

放到中文语境里，`regulatory-reporting` 不是一个独立应用，而是一段可以被 AI 助手调用的专业工作流。它把原来依赖人工经验的金融任务拆成更明确的触发条件、输入材料、处理步骤、输出格式和质量检查点。你阅读它时，不要只看英文标题，而要看它如何规定“先收集什么、再判断什么、最后交付什么”。
...
- 当你以后要把银行经验改造成中文 skill、提示词模板或自动化实验时，可以把 `regulatory-reporting` 作为结构参考。
...
- `Regulatory Reporting`（一级标题）：这是原始 skill 的结构节点，可作为阅读导航。
...
- `Form ADV Amendments`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `Form PF (Private Fund Reporting)`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `13F Filings (Institutional Holdings)`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `13H Filings (Large Trader Reporting)`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。

这些标题相当于原始 skill 的骨架。建议阅读顺序是：先看触发条件，再看步骤，再看示例，最后看风险/限制/参考文件。这样能避免只记住术语，却没有理解真实金融工作流。

## 7. 金融 + AI 的核心特点

`regulatory-reporting` 的核心特点，是把金融专业判断前面的准备工作结构化。AI 最适合做资料整理、格式化、初步检查、摘要、草稿、差异定位和问题清单；但金融结论本身仍然依赖数据质量、制度规则、业务经验和人工复核。
...
- `regulatory-reporting` 这个名称本身就是最重要的关键词。建议先拆开理解每个英文词，再结合所属领域判断它处理的是数据、流程、文档、合规还是客户沟通。
...
第一遍只读中文注释，理解 `regulatory-reporting` 的业务作用。第二遍打开原始 `SKILL.md`，对照每个标题看它怎样组织步骤。第三遍尝试把它改写成一个中文银行场景，例如“客户经理会前准备”“对账差错分析”“KYC 材料缺口识别”“财报摘要”或“市场早报”。
...
`regulatory-reporting` 的注释只解释该 skill 的工作流，不重复 01 模块里反洗钱/合规的制度总论；如果后续本地化，应另写中国银行业版本。
