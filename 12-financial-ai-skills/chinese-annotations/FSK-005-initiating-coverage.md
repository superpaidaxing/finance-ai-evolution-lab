# FSK-005 `initiating-coverage` 中文详细注释

## 1. 基本信息

- **Skill 编号**：FSK-005
- **原始名称**：`initiating-coverage`
- **所属领域**：股票研究与投研写作
- **来源项目**：`anthropic-financial-services`
- **原始路径**：`12-financial-ai-skills/upstream-snapshots/anthropic-financial-services/plugins/vertical-plugins/equity-research/skills/initiating-coverage/SKILL.md`

## 2. 这个 skill 是干什么的

原始描述可以理解为：Create institutional-quality equity research initiation reports through a 5-task workflow. Tasks must be executed individually with verified prerequisites - (1) company research, (2) financial modeling, (3) valuation analysis, (4) chart generation, (5) final report assembly. Each task produces specific deliverables (markdown docs, Excel models, charts, or DOCX reports). Tasks 3-5 have dependencies on earlier tasks.

股票研究和投研写作不是简单判断涨跌，而是围绕覆盖公司、行业数据、财务模型、业绩事件、催化剂、估值假设和风险因素形成可复核观点。

放到中文语境里，`initiating-coverage` 不是一个独立应用，而是一段可以被 AI 助手调用的专业工作流。它把原来依赖人工经验的金融任务拆成更明确的触发条件、输入材料、处理步骤、输出格式和质量检查点。你阅读它时，不要只看英文标题，而要看它如何规定“先收集什么、再判断什么、最后交付什么”。

## 3. 适合什么时候用

- 如果以后把 `initiating-coverage` 改造成中文 skill，应主动补充中文触发词，例如业务场景、材料名称、常见问题和输出形式。
- 当你想学习“股票研究与投研写作”这个领域的 AI 工作流拆解方式时，可以先读这个 skill。
- 当你以后要把银行经验改造成中文 skill、提示词模板或自动化实验时，可以把 `initiating-coverage` 作为结构参考。
...
- `Initiating Coverage`（一级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `⚠️ CRITICAL: One Task at a Time`（二级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `If User Requests Full Pipeline`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `Task Execution Rules`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `⚠️ Deliverables Policy: NO SHORTCUTS`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `Task Selection`（二级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `How to Use This Skill`（二级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `User Request Patterns and Responses`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。
- `Correct Usage Examples`（三级标题）：这是示例/场景部分，适合用来理解 skill 如何落到真实金融任务。
- `Task Execution Order`（三级标题）：这是原始 skill 的结构节点，可作为阅读导航。

这些标题相当于原始 skill 的骨架。建议阅读顺序是：先看触发条件，再看步骤，再看示例，最后看风险/限制/参考文件。这样能避免只记住术语，却没有理解真实金融工作流。

## 7. 金融 + AI 的核心特点

`initiating-coverage` 的核心特点，是把金融专业判断前面的准备工作结构化。AI 最适合做资料整理、格式化、初步检查、摘要、草稿、差异定位和问题清单；但金融结论本身仍然依赖数据质量、制度规则、业务经验和人工复核。
...
第一遍只读中文注释，理解 `initiating-coverage` 的业务作用。第二遍打开原始 `SKILL.md`，对照每个标题看它怎样组织步骤。第三遍尝试把它改写成一个中文银行场景，例如“客户经理会前准备”“对账差错分析”“KYC 材料缺口识别”“财报摘要”或“市场早报”。
...
`initiating-coverage` 的注释只解释公开市场/财富管理相关工作流，不重复投资建议或交易策略结论。
