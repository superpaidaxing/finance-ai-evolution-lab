...
第 2 批资源主要补足第 1 批中“可执行细分任务”的颗粒度：

| 新增资源 | 补充能力 | 为什么重要 |
| --- | --- | --- |
| FinQA / ConvFinQA | 财报数值推理、多轮计算问答 | 银行和财务分析中大量问题需要公式、口径和多步计算，不能只靠文本总结 |
| TAT-QA / MultiHiertt | 表格 + 文本混合推理 | 年报、授信材料和监管报表经常同时包含表格与解释性文本 |
| LOFin / FinDER | 长文档 RAG、证据检索 | 真实金融问答往往先难在“找对材料”，再难在生成答案 |
| Finance Agent Benchmark / Finova | 工具调用、Agent、安全合规 | 未来金融 AI 不是单轮聊天，而是可审计的工具化流程执行 |
| Financial PhraseBank / FiQA / FinSen | 金融情绪与市场信号 | 舆情、公告、新闻和市场波动分析是投研与风险预警的底层能力 |
| FinRED | 金融实体关系抽取 | 公司、产品、人员、交易、指标之间的结构化关系是知识图谱和风控分析基础 |

如果要设计自己的银行评测集，可以把第 2 批资源当成“题型库”：

1. 用 FinQA/TAT-QA 学习如何写带公式和证据的财务计算题。
2. 用 FinDER/LOFin 学习如何把制度、年报、监管文件转成 RAG 评测题。
3. 用 Finova/Finance Agent Benchmark 学习如何评估工具选择、执行轨迹和合规边界。
4. 用 Financial PhraseBank/FiQA/FinRED 学习如何构建情绪、关系抽取和事件归因的基础能力题。
